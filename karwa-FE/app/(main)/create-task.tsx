...
export default function CreateTaskScreen() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: Error) => {
      Alert.alert('Failed', error.message || 'Failed to create task. Please try again.');
    },
  });
  ...
  const handleChange = (field: keyof CreateTaskData, value: string | number | TaskType) => {
    ...
    if (errors[field]) {
      setErrors((prev: Partial<Record<keyof CreateTaskData, string>>) => ({ ...prev, [field]: undefined }));
    }
  };
          ...
          <View style={styles.form}>
              ...
              <View style={styles.typeContainer}>
                {TASK_TYPES.map((taskType) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.typeButton,
                      formData.type === taskType.value && styles.typeButtonActive,
                    ]}
                    onPress={() => handleChange('type', taskType.value)}>
                    <Text
                      style={[
                        styles.typeButtonText,
                        formData.type === taskType.value && styles.typeButtonTextActive,
                      ]}>
                      {taskType.label}
                    </Text>
                  </TouchableOpacity>
            ...
            <View style={styles.infoBox}>
              <Text style={styles.infoText}>
                Points will be automatically calculated based on the money amount.
              </Text>
            </View>
